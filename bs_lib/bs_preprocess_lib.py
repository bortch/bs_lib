...
def get_accuracy_curve(
    accuracy, val_accuracy=None, title="", filename=None, show=False
):
    plt.figure(figsize=(12, 7))
...
def get_multi_curve(
    histories, metrics=None, title=None, filename=None, show=False
):
    if not title:
    ...
    spec = gridspec.GridSpec(
        nrows=len(histories), ncols=len(metrics), figure=fig
    )
    ...
    for row, (key, history) in enumerate(histories.items()):
        ...
        for m in metrics:
            ...
            if m in history:
                ...
                ax.plot(history[key_metric_val], label="validation", color=c)
...
def multi_plot_best_results(
    data, params_values, compare=None, x_zoom=0, y_zoom=0, labels=None
):
    ...
    if isinstance(compare, np.ndarray):
    ...
    ax.annotate(
        f"Best score:{round(ymax,3)}\nBest value:{xmax}",
        xy=(xmax, ymax),
        xytext=(xmax, ymax + 0.05),
...
def get_silhouette(X, max_cluster=12, min_cluster=2, show=False):
    ...
    plt.scatter(
        x=[i for i in range(min_cluster, max_cluster + 1)],
        y=km_silhouette,
        s=150,
        edgecolor="k",
    )
